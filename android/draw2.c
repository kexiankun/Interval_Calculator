...
/* 获取当前目录+bmp */
char *Getthecurrentdirectorybmp(const char *filename)
{
...
//前景图片显示
 ...
 SDL_Renderer *renderer = SDL_CreateRenderer(win, -1, 1);
  ...
  ctx = nk_sdl_init(win);
  {
    ...
    //获取字库
    char *buffchar = Getthecurrentdirecyttf("/resources/方正宋三_GBK.ttf");
    struct nk_font *font =
