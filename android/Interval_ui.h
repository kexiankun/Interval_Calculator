...
void add_d(railings_data * d)
{

	d->interval = (d->tot_length - (d->railing_width * d->railing_root)) / (d->railing_root + 1);
}

void add_y(railings_data * y)
{

	y->interval = (y->tot_length - (y->railing_width * y->railing_root)) / (y->railing_root - 1);
}

void add_b(railings_data * b)
{
...
void *render_gui(struct nk_context *ctx, SDL_Window * win, int w, int h, int dpi)
{
	...
	if (nk_begin(ctx, "    间隔计算器v1.2", nk_rect(0, 0, w, h),
				 NK_WINDOW_CLOSABLE | SDL_WINDOW_MINIMIZED | NK_WINDOW_MINIMIZABLE |
				 NK_WINDOW_TITLE | NK_WINDOW_SCROLL_AUTO_HIDE
				 /* NK_WINDOW_NO_SCROLLBAR */ ))
	{
		enum
		{ One0, Two1, Three2, Four3 };
		static int op = One0;
		enum
		{ One, Two, Three, Four, Zz };
		static int lop = Zz;
		...
		if (nk_button_label(ctx, "开始计算"))
		{
			...
			*current2 = atof(buf_railing_root);
			...
			/* if (op == One0) { add_d(&data); } if (op == Two1) {
			   add_y(&data); } if (op == Three2) { add_b(&data); } if (op ==
			   Four3) { add_dd(&data); } */
		...
		if (popup_active)
		{
			static struct nk_rect s = { 80, 1000, 910, 600 };
			if (nk_popup_begin(ctx, NK_POPUP_STATIC, "root", 0, s))
			{
				nk_layout_row_dynamic(ctx, RH / 2, 1);
				nk_label(ctx, "使用说明：看png图解 ", NK_TEXT_LEFT);
				nk_label(ctx, "1：软件是免费的，可以复制，使用，传播",
						 NK_TEXT_LEFT);
				nk_label(ctx, "2：软件版权归@ by：jnck所有 微信：snd8899", NK_TEXT_LEFT);
                                nk_label(ctx,"3：v1.2_20220503_beta", NK_TEXT_LEFT);
				
				//1.0.1.20220503_beta

				//nk_label(ctx, "作者：Jnck WX：snd8899\0ghjhh ",
				 //NK_TEXT_LEFT);
				...
				/* if (nk_button_label(ctx, "取消")) { popup_active = 0;
				   nk_popup_close(ctx); } */
				nk_popup_end(ctx);
			}
			else
		...
		nk_flags event = nk_edit_string_zero_terminated(ctx, NK_EDIT_SIMPLE, buf_tot_length, 36,
														nk_filter_float);
		...
		nk_flags event1 =
			nk_edit_string_zero_terminated(ctx, NK_EDIT_SIMPLE, buf_railing_width, 36,
										   nk_filter_float);

		if (event1 & NK_EDIT_ACTIVATED)
		{
			SDL_StartTextInput();
			nk_style_item_color(nk_rgb(222, 22, 111));
		}
		...
		nk_flags event2 = nk_edit_string_zero_terminated(ctx, NK_EDIT_SIMPLE, buf_railing_root, 36,
														 nk_filter_float);
		...
		nk_flags event3 =
			nk_edit_string(ctx, NK_EDIT_CLIPBOARD, buf_interval, &len, 256, nk_filter_float);
		/* if (event3& NK_EDIT_ACTIVATED) { SDL_StartTextInput(); } if
		   (event3& NK_EDIT_DEACTIVATED) { SDL_StopTextInput(); } */

		/* 函数;单选框

		   nk_layout_row_dynamic(ctx, RH / 2, 3);

		   if (nk_option_label(ctx, "两边有间隔", op == One0)) { op =
		   One0; } if (nk_option_label(ctx, "两边没有间隔", op == Two1)) 
		   { op = Two1; } if (nk_option_label(ctx, "单边有间隔", op ==
		   Three2)) { op = Three2; } // if (nk_option_label(ctx,
		   "总长等分", op == Four3)) { op = Four3; } */
		...
		if (nk_combo_begin_label(ctx, items[selected_item], nk_vec2(nk_widget_width(ctx), RH * 3)))
		{
			...
			{
				if (nk_combo_item_symbol_label(ctx, gg, items[i], NK_TEXT_RIGHT))
				{

					// if (nk_combo_item_label(ctx, items[i],
					// NK_TEXT_CENTERED))
					selected_item = i;
				}
			}
		...
		/* static float value = 0.2f; //滚动条 nk_layout_row_begin(ctx,
		   NK_STATIC, RH / 2, 2); { nk_layout_row_push(ctx, 180);
		   nk_label(ctx, "调整屏幕亮度:", NK_TEXT_LEFT);
		   nk_layout_row_push(ctx, 1400); nk_slider_float(ctx, 0, &value,
		   5.0f, 0.6f); } */
		...
		/* nk_layout_row_push(ctx, RH); if (nk_button_label(ctx, "关于:"))

		   popup_active = 1; nk_layout_row_end(ctx); if (popup_active) {
		   static struct nk_rect s = {280, 600, 500, 300}; if
		   (nk_popup_begin(ctx, NK_POPUP_STATIC, "Error", 0, s)) {
		   nk_layout_row_dynamic(ctx, RH / 2, 1); nk_label(ctx,
		   "发生了未知错误!!", NK_TEXT_LEFT);
		   nk_layout_row_dynamic(ctx, RH / 3, 2); if (nk_button_label(ctx,
		   "确定")) { popup_active = 0; nk_popup_close(ctx); } if
		   (nk_button_label(ctx, "取消")) { popup_active = 0;
		   nk_popup_close(ctx); } nk_popup_end(ctx); } else popup_active =
		   nk_false; } */
	}
